...
Companies, teams, and projects provide paginated relation options for the editors. The client follows each opaque cursor and caches the completed option list for the authenticated session. Task and Work Log JIRA selection is the deliberate exception: both editors use the lightweight `QUERY /api/jiras/options` picker described below and never load the full JIRA relation collection.
...
`GET /api/jiras/:jiraKey?include=relations` also returns `timeline`, chronological `workLogs` and `workLogCount`, and historical `releaseItems` and `releaseItemCount`; the JIRA page does not make separate Work Log or Release calls. `timeline.endedDate` is the latest related Sprint end date, not a completion date. `GET /api/jiras/meta?mode=edit` provides the edit-only writable metadata. `PATCH /api/jiras/:jiraKey` accepts a partial body only. The editor sends only changed controls; rich-text description changes are sent as `descriptionRichTextHtml`, which the Worker converts to stored Markdown.
