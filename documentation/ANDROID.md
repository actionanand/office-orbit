...
Worker access tokens use native Keystore-backed encrypted storage. Login completes only after the token is durably saved; a transient native read failure shows startup retry instead of incorrectly opening Login. Tokens never fall back to browser storage. PIN records contain only a salted verifier and may fall back to the app-private IndexedDB used by the Life Leaf security pattern. Both Android backup and device-transfer policies exclude application data. Release cleartext HTTP is disabled. The app has no Notion credentials or Worker signing secrets.

Native Work Log PDF export follows the working Office Pulse bridge: `OfficeOrbitExportPlugin` writes into the app cache and shares the PDF through Capacitor's existing `${applicationId}.fileprovider` authority and `file_paths.xml`. The Android chooser lets the user save or share the resulting file without storage permissions. The export call has a 20-second client deadline and displays the native failure reason when Android rejects an export.
...
No packages were installed by the agent and no app/release build was run. TypeScript validation passes, all 76 Vitest tests pass, and all four Android script tests pass under the project's installed Node 24 WSL environment. The generated Android patch was applied successfully. Physical-device behavior still requires verification with the next APK. See [Android security acceptance checks](android-security.md#device-acceptance-checks).
