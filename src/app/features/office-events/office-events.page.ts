...
import { EVENT_LABELS, OfficeEvent, Period, eventsInWindow, monthWindow, sprintWindow } from './office-event';
            ...
            @if (!visibleEvents().length) {
              ...
              <ol class="office-event-list" aria-label="Office events in chronological order">
                @for (event of visibleEvents(); track event.id) {
                  <li class="office-event-row">
                    <div class="office-event-date">{{ eventDate(event) }}</div>
                    <div class="office-event-body">
                      <span class="status-badge">{{ labels[event.type] }}</span>
                      <h2>{{ event.title }}</h2>
                      @if (event.day || event.tamilDay) {
                        <p>{{ event.day }}{{ event.day && event.tamilDay ? ' · ' : '' }}{{ event.tamilDay }}</p>
                      }
                      @if (event.othersInvolved) {
                        <p>With: {{ event.othersInvolved }}</p>
                      }
                      @if (event.comments) {
                        <p>{{ event.comments }}</p>
                      }
...
export class OfficeEventsPage {
  ...
  readonly visibleEvents = computed(() => {
    ...
    return window ? eventsInWindow(this.events(), window) : [];
  });
